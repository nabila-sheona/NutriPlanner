...
const RecipeUploadsHeatmap = ({ heatmapSize = 800 }) => {
  ...
  // New start date = Jan 1 current year
  const getStartDate = () => {
    const date = new Date();
  ...
  // New end date = Dec 31 current year
  const getEndDate = () => {
    const date = new Date();
    date.setMonth(11); // December
    date.setDate(31); // 31st
  ...
  return (
    <Container maxWidth="lg" sx={{ mt: 4, mb: 4 }}>
      ...
      >
        {/* <Typography variant="h4" component="h1" gutterBottom>
          Recipe Uploads Heatmap ({new Date().getFullYear()})
        </Typography> */}
            ...
            <Box sx={{ overflowX: "auto", py: 2 }}>
              <CalendarHeatmap
                ...
                tooltipDataAttrs={(value) => ({
                  "data-tooltip-id": "heatmap-tooltip",
                  "data-tooltip-content": value?.date
                    ? `${formatDate(value.date)} — ${value.count} upload${value.count !== 1 ? "s" : ""}`
                    : "No uploads",
                })}
                classForValue={(value) => {
                  if (!value || !value.count) return "color-empty";
